...
As we know, Android normally ships with the sh(Bourne Shell) which requires its own Algorithms, depends on PATH, external utilities, 
or programs available because it doesn't have all the general commands in need. However, when system fails or if the core libraries
get corrupted or if a part of the filesystem becomes totally unmountable, programs dependent on other programs will fail to load. In
this scenario, sash which has all libraries statically linked, even dynamically linked will not fail and can help in various circumstances
of development. Now, many know that there exists busybox but Busybox is not a shell but a software suite that provides several Unix 
utilities in a single executable file, that means busybox needs to be invoked every time a shell prompts up. However, like every shell
it contains built-in commands but not every command. Thus other utilities like busybox can help.
